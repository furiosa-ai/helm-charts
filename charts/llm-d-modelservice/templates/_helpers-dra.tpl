...
{{- if and .role (eq .role "decode") -}}
  ...
  {{- else -}}
    {{- .Values.accelerator.type | default "furiosa" -}}
  {{- end -}}
{{- else if and .role (eq .role "prefill") -}}
...
{{- if hasKey .Values.accelerator.resourceClaimTemplates $acceleratorType -}}
  ...
  {{- else -}}
    {{- /* Auto-calculate the Furiosa RNGD count for this pod. */}}
    {{- $count = int (include "llm-d-modelservice.furiosaRngdCountPerPod" .parallelism) -}}
  {{- end -}}
{{- else -}}
  {{- $count = int (include "llm-d-modelservice.furiosaRngdCountPerPod" .parallelism) -}}
{{- end -}}
